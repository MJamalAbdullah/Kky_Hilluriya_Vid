...
<div class="container-fluid">
    <div class="card shadow mb-4">
        ...
        <h6 class="m-0 font-weight-bold text-primary">papers_edit_staff Edit </h6>
        </div>
        <div class="card-body">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label >Subject :</label>
                    <input class="form-control" type="text" paceholder="Name" autocomplete="off" value="<?php echo $row['subject'];?>" name="name" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <center><a href="pdf/<?php echo $row['file']; ?>" target="_blank"><?php echo $row['file']; ?></a><br></center>
                    <!-- <img class="img-profile "  alt="Image" src="pdf/<?php //echo $row['file']; ?>" width="200px" height="200px"> -->
                </div>
                    
                <div class="form-group" align="center">
                    <a class="btn btn-secondary" href="pastpapers.php">Cancel</a>
...
<?php
    if(isset($_POST['save']))
    {
        ...
        move_uploaded_file($_FILES["img"]["tmp_name"], "pdf/".$_FILES["img"]["name"]);

        $sql="UPDATE papers SET subject='$name',description='$desc',file='$img_data' WHERE id='$id';";
        $res=mysqli_query($con,$sql);

        if($res)
        {
            echo "<script> alert('File Updated Successfully.'); location.replace('./pastpapers.php') </script>";
        }
        else
        {
    ...
    if(isset($_POST['savepw']))
    {
        ...
        if ($decryption_pass == $cpw)
        {
            if($npw == $rnpw)
            {
                ...
                if($respw)
                {
                    echo "<script> alert('Password Successfuly Changed.'); location.replace('./pastpapers_edit_staff.php') </script>"; 
                }
            ...
            {
                echo "<script> alert('Please Verify The Both Passwords are Same.'); location.replace('./pastpapers_edit_staff.php') </script>";
            }
        }
        else
        {
            echo "<script> alert('Please Verify The Current Password is Correct.'); location.replace('./pastpapers_edit_staff.php') </script>";
        }       
    }
