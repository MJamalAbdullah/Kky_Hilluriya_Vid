    ...
    <div class="row">
        <div class="col-md-5 border-right ">
            <form id="form" enctype="multipart/form-data" method="post">
                <div class="p-3 py-5">
                    <div class="d-flex flex-column align-items-center text-center"><img height="75%" width="75%" id="img_pre"style="margin-top:50%" width="400px" src="img/<?php echo $row['image']; ?>"><span class="font-weight-bold"><?php echo $fname." ".$lname; ?></span><span class="text-black-50"><?php echo $email; ?></span><span> </span></div>
                    <div class="col-md-12"><label class="labels">User Imge</label><input type="file" class="form-control"  name="dp" accept="image/*"></div>
                    <!-- <div class="mt-5 text-center"><button class="btn btn-primary profile-button" type="submit" name="dpchange">Change Profile</button></div> -->
                </div>
        </div>
...
<?php
    if(isset($_POST['save']))
    {
        ...
        if(isset($_FILES['dp']))
        {
            ...
            if(!$_FILES['dp']['size'] <= 3072)
            {
                ...
                $sqlup="UPDATE user SET First_Name='$firstnm', Last_Name='$lastnm',dob='$dobd', Gender='$gender',email='$emailadd', phone='$mobile', index_or_staff='$index',NIC='$nic',image='$img_data' WHERE username='$user';";
                $exeup=mysqli_query($con,$sqlup);
            
                if($exeup)
                {
                    echo "<script> alert('Your Profile Updated .'); location.replace('./profile.php'); </script>";
                }
                else
                {
            ...
            {
                echo "<script> alert('The File Size Is to Large...! Please Upload Lessthen 3MB.'); location.replace('./profile.php') </script>";
            }
        }
        
        // $sqlup="UPDATE user SET First_Name='$firstnm', Last_Name='$firstnm',dob='$dobd', Gender='$gender',email='$emailadd', phone='$mobile', index_or_staff='$index',NIC='$nic' WHERE username='$user';";
        // $exeup=mysqli_query($con,$sqlup);
    ...
    //             echo $_FILES["dp"]["name"];
                ...
                // unlink($img_path);
                // $img_data=$_FILES["dp"]["name"];

                // move_uploaded_file($_FILES["dp"]["tmp_name"], "index_page_img/".$_FILES["dp"]["name"]);

                // $sqlup="UPDATE user SET image='$img_data' WHERE username='$user';";
                // $exeup=mysqli_query($con,$sqlup);

                // if($exeup)
                // {
                //     echo "<script> alert('User Image Updated Successfully.'); location.replace('./profile.php') </script>";
                // }
                // else
                // {
                //     die(mysqli_error($con));
    ...
    //         else
    //         {
    //             echo "<script> alert('The File Size Is to Large...! Please Upload Lessthen 5MB.'); location.replace('./profile.php') </script>";
    //         }
    // }
?>
<!-- ------------------------------------------------------------------------------------------------------------------------------------------------ -->
